    ...
    document.querySelectorAll(".a").forEach((btn) => {
        btn.addEventListener("click", function (e) {
            for (let i = 0; i < 6; i++) {
                ...
                // Posicionamos el corazón en la ubicación del clic, asegurándonos de que estén dentro del botón
                let rect = btn.getBoundingClientRect();
                let x = e.clientX - rect.left;
                let y = e.clientY - rect.top;

                heart.style.left = `${x - 15}px`; // Ajustamos para que el corazón esté centrado en el clic
                heart.style.top = `${y - 15}px`; // Ajustamos para que el corazón esté centrado en el clic

                // Añadimos el corazón al botón
                btn.appendChild(heart);

                // Quitamos el corazón después de que termine la animación
                setTimeout(() => {
                    heart.remove();
                }, 1500); // El mismo tiempo que dura la animación
            }
        });
    });
});
